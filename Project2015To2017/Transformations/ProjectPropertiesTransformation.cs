...
namespace Project2015To2017.Transformations
{
	...
	{
		private readonly List<string> _webApplicationGuids = new List<string>
		{
		...
		public Task TransformAsync(XDocument projectFile, DirectoryInfo projectFolder, Project definition, Settings settings)
		{
			...
			definition.ApplicationInsightsResourceId = unconditionalPropertyGroups.Elements(nsSys + "ApplicationInsightsResourceId").FirstOrDefault()?.Value;
			...
			else if (unconditionalPropertyGroups.Elements(nsSys + "ProjectTypeGuids").Any(
				e => _webApplicationGuids.Any(wag=> e.Value.IndexOf(wag, StringComparison.OrdinalIgnoreCase) > -1)))
			{
				definition.Type = ApplicationType.WebApplication;
			}
			else
			{
				definition.Type =
